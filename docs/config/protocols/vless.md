...
1. VLESS 协议本身还会有不兼容升级，但客户端配置文件参数基本上是只增不减的。</br>
所以如果你开发了用 core 的客户端，现在就可以适配。iOS 客户端的协议实现则需紧跟升级。

2. UI 标识请统一用 VLESS，而不是 VLess / Vless / vless，配置文件不受影响，代码内则顺其自然。

3. encryption 应做成输入框而不是选项框，默认值为 `none`，若用户留空则应代填 `none`。

### 以下为已支持图形化设置 VLESS 的部分客户端列表，推荐使用：（按时间先后顺序排列）
