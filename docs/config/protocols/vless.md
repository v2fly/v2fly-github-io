...
**最新的 v4.32.1 版本中，VLESS XTLS Direct Mode 引入了 ReadV 增强，减少一层内存 copy，性能已与 VLESS 无加密裸奔持平（接近于纯流量转发），为传统 VMess WSS 方案的五倍、VLESS TCP TLS 的三倍（且测试机器 CPU 均有 AES 指令集，否则差距更大），强烈建议测试体验。**
