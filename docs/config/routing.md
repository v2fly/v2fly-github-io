...
可以用于选择要使用的域名匹配算法。 留空则使用默认的域名匹配算法。

设置为 `hybrid` 时，则使用基于混合 AC 自动机算法的域名匹配算法。(v4.35.1+)

设置为 `linear` 或 留空 时, 则使用传统的线性匹配算法。此算法是默认的域名匹配算法。
...
对应一个 [额外出站连接配置](protocols.md) 的标识。
...
<<<<<<< HEAD
可以用于选择要使用的域名匹配算法。 留空则使用默认的域名匹配算法。此设置可以覆盖 `RoutingObject` 处的设置。

设置为 `hybrid` 时，则使用基于混合 AC 自动机算法的域名匹配算法。(v4.35.1+)
=======
选择要使用的域名匹配算法，不设置或留空时使用默认的域名匹配算法。
>>>>>>> f434134e0f1718adc24e57a51f692ad8d544fc15
